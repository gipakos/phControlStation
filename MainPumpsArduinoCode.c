...
void loop() {
  ...
  while(1){
    ...
    case STEP_START_PUMP1:
      ...
      Serial.println("Mixing motor is active...");
      digitalWrite(mixingMotorPin, LOW); //ANTISTROFO -> RELAY = HIGH

      currentStep = STEP_STOP_MIXING_MOTOR;
      }
      ...
      Serial.println("Mixing motor is now stopped...");
      digitalWrite(mixingMotorPin, HIGH); //ANTISTROFO -> RELAY = LOW
      ...
      if (currentMillis - previousMillis >= 5000) {
        ...
        if(phValue >= phNeutralValue){
          Serial.println("Solution is OK");
          Serial.println("Starting PUMP2...");
          delay(2000);
          currentStep = STEP_START_PUMP2;
        } else if(phValue < phNeutralValue){
          Serial.println("Solution needs correction, starting peristaltic pump");
          delay(2000);
      ...
      Serial.println("Peristaltic pump is now running...");
      ...
      Serial.println("Waiting the peristaltic pump to stop...");
      ...
      Serial.println("Waiting for pump 2 to stop...");

      delay(2000);

      Serial.println("Pump 2 is stopping...");
